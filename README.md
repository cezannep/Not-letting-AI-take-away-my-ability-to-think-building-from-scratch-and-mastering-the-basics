# Zance UI

A inspired futuristic animated UI experience built with React, TypeScript, Vite, Tailwind CSS, and Motion.

---

##  Tech Stack

- React 19
- TypeScript
- Vite
- Tailwind CSS v4
- Motion
- ESLint

---

## 📦 Installation

Clone the repository:

```bash
git clone https://github.com/cezannep/Not-letting-AI-take-away-my-ability-to-think-building-from-scratch-and-mastering-the-basics.git
```

Navigate into the project:

```bash
cd "zance ui"
```

Install dependencies:

```bash
npm install
```

---

## 🛠️ Run Development Server

```bash
npm run dev
```

---

## 📦 Production Build

Build the project:

```bash
npm run build
```

---

## 🧹 Lint Project

```bash
npm run lint
```

---

## ⚡ Features

- Smooth animations using Motion
- Responsive futuristic UI
- Fast development with Vite
- Type-safe architecture with TypeScript
- Tailwind CSS styling system
- Clean scalable component structure

---

## 🖼️ UI Preview
Hover Me!
<img width="1920" height="961" alt="Screenshot from 2026-05-17 23-53-28" src="https://github.com/user-attachments/assets/9e0837a4-724e-441e-9986-f354d252c882" />
<img width="1920" height="961" alt="Screenshot from 2026-05-17 23-53-37" src="https://github.com/user-attachments/assets/aa0db30f-b829-4ec2-be87-353c990f68a5" />
<img width="1920" height="961" alt="Screenshot from 2026-05-17 23-53-49" src="https://github.com/user-attachments/assets/94104ef9-397e-4c2a-bc6c-2fdc1824a9e1" />


---

## 📄 License

MIT License

---

## 👨‍💻 Author

Built by Cezanne P.
